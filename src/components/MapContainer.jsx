	...
	useEffect(() => {
		...
		if (isFiltered) {
			locationsToShow = locationsToShow.filter(([locationName, data]) => {
				...
				const [minElevation, maxElevation] = selectedOptions.elevation
					.split("-")
					.map(Number);
		...
		const newMarkers = locationsToShow.map(([locationName, data]) => {
			...
			marker.on("click", () => {
                ...
                if(lastSelectedMarker){
                    //On click the selected marker will be deseleceted
                    if(lastSelectedMarker._leaflet_id ===  marker._leaflet_id){
